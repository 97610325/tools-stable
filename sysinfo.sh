...
detect_virtualization_architecture() {
    ...
    echo "$virtualization_architecture"
}
...
echo -e "${mikublue}内存: ${white}$used_mem/${yellow}$total_mem ${white}($used_percentage%) ${mikublue}SWAP(虛擬內存):${white}$swap_memory ${mikublue}硬盘:${white}$disk_usage"
